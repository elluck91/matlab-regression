function logCarryPositionAccuracy(algorithmData)
    if ~strcmp(algorithmData.gt,'NA')
        if ~exist(algorithmData.accuracyLogFile,'file')
            ...
            header = ['FileName, GroundTruth, ' ...
                ...
                'OnDesk, InHand, NearHead, ShirtPocket, TrouserPocket, ArmSwing, JacketPocket,\n'];
            fprintf(fid, header);
        ...
        if iscell(fileContent)
            ...
            fprintf(fid,'%s, %d, %.2f, %s, %d, %d, %d, %d, %d, %d, %d, %d, %d\n', ...
                ...
                algoResults(1, 6), algoResults(1, 7), algoResults(1, 8));
...
function [results, sampleSize] = getAlgoResults(outputDir)
    ...
    if ~sampleSize
        error('No activity discovered. Carry Position Test cannot be performed.');
    end
