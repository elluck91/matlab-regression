function [matrix, combinedMatrix]= ARMetricsGen(smartActivity, results, gt, settings)
    combinedCurrentMatrix=[results '/Combined_ConfusionMatrix.csv'];
    combinedMatrix=[fileparts(settings.output) '/Combined_Confusion_Matrix_ActivityRecognition_PDR_Data.csv'];

    %smartalgo=importdata(smartActivity,',');
    %algoactivitycode=smartalgo.data(:,2);
    %smartgtactivitycode=smartalgo.data(:,3);
    %Noofsamples=length(smartalgo.data(:,2));
    ...
    combined_confmat_current = zeros(14,7);

    if ~exist(combinedMatrix,'file')
        combined_confmat_overall = zeros(14,7);
    else 
    ...
    fprintf(fid,'Source, Activity 0, Activity 1, Activity 2, Activity 3, Activity 4, Activity 5, Activity 6\n');
