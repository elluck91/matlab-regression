%{
% Synopsis:
%[textObj,patchObj,nodeNames,adjacent]=mGraphViz(dotFile,surface,parameters)
% INPUT ARGUMENTS:
...
% drawing.
%
% The following parameters are available to be passed as parameter/value
% pairs:
%  o colourMode
%  o arrowStyle
%  o layoutEngine
...
%
% colourMode
%  specifies whether colour triplets are RGB or hsv triplets,
%  by setting it to either 'hsv' or 'rgb'. Default assumes RGB. Ex:
%  'colourMode','rgb' in the arguments.
%
% arrowStyle
%  lets you specify the arrow style to include on the graph,
...
%  additionally use '<'/'>' to specify reverse/forward arrows.
%  specifying both gives two-directional arrows.
%  For example: '<normal','>normal','<>normal'
%
% layoutEngine
%  mGraphViz will always use the mwdot engine
%
% autoSize
%  when true, automatically sets the figure's window to the
...
%
% edgeResolution
%  specifies the number of points to use for each B-spline
...
%
% fullMode
%  (true/false) enables/disables all features of the plot routines.
%  (Enabled by default). Disabling it offers little performance gain.
%  The resulting graph has no patches for node shapes, using instead the
%  border of the text object to draw a box around each piece of text.
%  This makes "interactive" graphs easier to implement, since the box will
%  always fit around the text, even if it is changed after the original plot
%  has completed. Zooming and text scaling features are also disabled when
%  fullMode is set to false.
%
% OUTPUT:
% textObj
%     returns the handles to the _node_ text objects in the graph.
% patchObj
%     returns the handles to each node's patch, in the same order.
% nodeNames
%     returns the names of each node, in the same order as the above.
% adjacent
%     returns an adjacency matrix for the graph. Finding a '1' in
%     row M, column N of the adjacency matrix means that M is a parent of N,
%     or conversely, that N is a child of M.
%
% This code loads a DOT file without coordinates given, passes it to
% mwdot, and draws a graph in the axis specified.
% simple graphs are currently supported
%
% The original interface was written by Bjorn Wielens and released as
%  drawDot.m - Copyright 2009.
%
...
function [textObj,patchObj,nodeNames,adjacent]=mGraphViz(dotFile,surface,varargin)
%arrow('clear');
%argument checking
...
if mod(numel(varargin),2)~=0
    error('mGraphViz:MissingArguments',...
        'You do not have a complete set of parameter/value arguments');
end
...
%check and set parameter argument pairs
...
if status~=0
    error('mGraphViz:EngineError',...
        ['An error occurred when trying to run the layout engine. ' ...
        'The system said, "' returnCode '".']);
end

%clear the axes that the graph will be drawn on
cla(surface);

%determine the parent figure of the graph's axes
...
axes(surface); %#ok<MAXES>

%stretch the axes out over the entire surface of the figure
set(surface,'units','normalized','position',[0 0 1 1]) %added mGraphViz

%hold is required to draw several plots on the same figure
hold on;

scale=str2double(tokens{2});

%horizontal offset of node
hOffset=str2double(tokens{3})*scale*0.05;

%vertical offset of node
vOffset=str2double(tokens{4})*scale*0.05;

axis([0-hOffset (str2double(tokens{3})*scale)+vOffset...
    0-vOffset (str2double(tokens{4})*scale)+vOffset]);

% remove the tick labels
set(surface,'XTick',[],'YTick',[]);

% This is limiting the figure to the screen size
if autoSize
    set(figureHandle,'units','inches','position',...
        [4 4 str2double(tokens{3})*scale str2double(tokens{4})*scale]);
    set(figureHandle,'units','pixels');
    centerfig(figureHandle);
end

%=========================================================================%
%                      NODES (Shapes and Textboxes)                       %
%=========================================================================%
...
%get the edge colors
edgeColours=cellfun(@(a)getColour(a,colourMode),...
    tokens(:,9),'UniformOutput',false);

%get the background colors
bgColours=cellfun(@(a)getColour(a,colourMode),...
    tokens(:,10),'uniformOutput',false);

%replace linestyle values with valid matlab styles
properties=regexprep(tokens(:,7),{'dashed','solid','dotted','bold',...
    'invis','filled','diagonals'},{'lineStyle','lineStyle','lineStyle',...
    'lineWidth','visible','faceColor','lineStyle'});

values=regexprep(tokens(:,7),{'dashed','solid','dotted','bold',...
    'invis','filled','diagonals'},{'--','-',':','bold','off',...
    'faceColour','diags'});

%test for bold setting
boldReplace=~cellfun('isempty',regexp(values,'bold'));

%test for face color setting
colourReplace=~cellfun('isempty',regexp(values,'faceColour'));

blackEdges=cellfun(@(a)all(~a),edgeColours);

%diaginal is not supported change to dash-dot line
diagReplace=~cellfun('isempty',regexp(values,'diags'));
if any(diagReplace)
    warning('mGraphViz:NotSupported',...
        'Diagonal styles are not supported. Defaulting to dash-dot line.');
    values(diagReplace)='-.';
end

blackEdges=and(colourReplace,blackEdges);

%make replacements in the cell array
values(colourReplace)=edgeColours(colourReplace);
values(blackEdges)=bgColours(blackEdges);
values(boldReplace)={2};

% fullMode creates a patch object for the shape of the node, if one were to
% limit the shapes to only boxes, then full mode would not be required

if fullMode
    [patchObj]=cellfun(@drawNode,shapes,num2cell(x),num2cell(y),...
        num2cell(w),num2cell(h),edgeColours,bgColours,properties,...
        values,'UniformOutput',false);
    textObj=text(x,y,labels,'HorizontalAlignment','center',...
    ...
    textObj=text(x,y,labels,'HorizontalAlignment','center',...  %mGraphViz
        'Interpreter','none','verticalAlignment','middle',...
        'margin',2,'edgeColor',[0 0 0]);
end

%=========================================================================%
%                      EDGES (lines and arrows)                           %
%=========================================================================%
...
splinePts=cellfun(@(a,b)reshape(a*scale,[],b),splinePts,numSpline,...
    'uniformoutput',false);

splinePts=cellfun(@horzcat,num2cell(tailCoord,1)',splinePts,...
    num2cell(tipCoord,1)','Uniformoutput',false);

space=repmat(linspace(0,1,edgeResolution),numel(splinePts),1);

splinePts=cellfun(@transpose,splinePts,'uniformoutput',false);

[NULL,NULL,splinePts]=cellfun(@bezier,...
    num2cell(zeros(numel(splinePts),1)),...
    num2cell(ones(numel(splinePts),1)),splinePts,...
    num2cell(space,2),'Uniformoutput',false);
...
if ~isempty(labels)
    ...
    if any(~empty)
        ...
        text(labelX(~empty),labelY(~empty),labelText(~empty),...
            'HorizontalAlignment','center','verticalAlignment',...
            'middle','interpreter','none');
    end
end

colours=cellfun(@(a)getColour(a,colourMode),...
    tokens(:,7),'UniformOutput',false);

properties=regexprep(tokens(:,6),...
    {'dashed','solid','dotted','bold','invis'},...
    {'lineStyle','lineStyle','lineStyle','lineWidth','lineStyle'});

values=regexprep(tokens(:,6),...
    {'dashed','solid','dotted','bold','invis'},...
    {'--','-',':','bold','none'});
...
% limit the shapes to only boxes, then full mode would not be required
if fullMode
    % get coordinates of patch object to connect edge to
    % tip
    ...
    % get cooridinates of textbox object to connect edge to
    % tip
    tipPoly=get(textObj(tipLocation),'Extent');
    if ~isa(tipPoly,'cell'); tipPoly = {tipPoly}; end
    
    tipPolyX=cellfun(@(a) [a(1) a(1) a(1)+a(3) a(1)+a(3)],...
        tipPoly,'uniformoutput',false);   
    
    tipPolyY=cellfun(@(a) [a(2)+a(4) a(2) a(2) a(2)+a(4)],...
        tipPoly,'uniformoutput',false);
    
    
    % tail
    tailPoly=get(textObj(tailLocation),'Extent');
    if ~isa(tailPoly,'cell'); tailPoly = {tailPoly}; end
    
    tailPolyX=cellfun(@(a) [a(1) a(1) a(1)+a(3) a(1)+a(3)],...
        tailPoly,'uniformoutput',false);
    
    tailPolyY=cellfun(@(a) [a(2)+a(4) a(2) a(2) a(2)+a(4)],...
        tailPoly,'uniformoutput',false);
    
end

% force to cell arrays if the tip and tail X and Y are not
if ~isa(tipPolyX,'cell'); tipPolyX = {tipPolyX}; end
if ~isa(tipPolyY,'cell'); tipPolyY = {tipPolyY}; end
if ~isa(tailPolyX,'cell'); tailPolyX = {tailPolyX}; end
if ~isa(tailPolyY,'cell'); tailPolyY = {tailPolyY}; end


% test for Edge in region of Node polygon
inPolygon=cellfun(@inpolygon,splineX(~multiColour),...
    splineY(~multiColour),tipPolyX(~multiColour),...
    tipPolyY(~multiColour),'uniformoutput',false);

% test for Edge in region of Ending Node polygon
inPolygon2=cellfun(@inpolygon,splineX(~multiColour),...
    splineY(~multiColour),tailPolyX(~multiColour),...
    tailPolyY(~multiColour),'uniformoutput',false);

% test for Edge in region of Starting Node polygon
inPolygon=cellfun(@not,cellfun(@or,inPolygon,inPolygon2,...
    'uniformoutput',false),'uniformoutput',false);

% bring together
splineX(~multiColour)=cellfun(@(a,i) a(i),splineX(~multiColour),...
    inPolygon,'uniformoutput',false);

splineY(~multiColour)=cellfun(@(a,i) a(i),splineY(~multiColour),...
    inPolygon,'uniformoutput',false);
% plot connecting lines
cellfun(@(a,b,c,d,e)plot(a,b,c,d,'Color',e),splineX(~multiColour),...
    splineY(~multiColour),properties(~multiColour),...
    values(~multiColour),colours(~multiColour));

if any(multiColour)
    [splineX(multiColour),splineY(multiColour)]=cellfun(@multi_Coords,...
        splineX(multiColour),splineY(multiColour),colours(multiColour),...
        tipPolyX(multiColour),tipPolyY(multiColour),...
        tailPolyX(multiColour),tailPolyY(multiColour),...
        'uniformoutput',false);
    
    cellfun(@multi_Plot,splineX(multiColour),splineY(multiColour),...
        properties(multiColour),values(multiColour),colours(multiColour));
end

% check for forward arrow head  (ending arrow head)
...
% check for reverse arrow head (starting arrow head)
if ~isempty(regexp(arrowStyle,'<','once'))
    revArrow=true;
else
    revArrow=false;
...
% create arrows and arrow heads
arrowStyle=regexprep(arrowStyle,'[\<\>]','');

noLines=or(cellfun('length',splineX)<2,cellfun('length',splineY)<2);

splineX(multiColour)=cellfun(@getPoints,splineX(multiColour),...
    'uniformoutput',false);

splineY(multiColour)=cellfun(@getPoints,splineY(multiColour),...
    'uniformoutput',false);

colours(multiColour)=cellfun(@cell2mat,colours(multiColour),...
    'uniformoutput',false);

% reverse arrow i.e. A <-- B
if revArrow
    arrowX=cellfun(@(a) a(end,[1 2]),splineX(~noLines),...
        'uniformoutput',false);
    arrowY=cellfun(@(a) a(end,[1 2]),splineY(~noLines),...
        'uniformoutput',false);
    cellfun(@(a,b,c) arrow(arrowStyle,a(1,:),b,c),...
         colours(~noLines),arrowX,arrowY);
end

% forward arrow i.e.  A --> B
if fwdArrow
    arrowX=cellfun(@(a) a(1,[end end-1]),splineX(~noLines),...
        'uniformoutput',false);
    arrowY=cellfun(@(a) a(1,[end end-1]),splineY(~noLines),...
        'uniformoutput',false);
    cellfun(@(a,b,c) arrow(arrowStyle,a(end,:),b,c),...
         colours(~noLines),arrowX,arrowY);
    % 
    %  Other modifications have redered the following unnecessary
    %   Replaced the arrow funciton with the arrowh function from the file exchange
    %   need to vectorize or use cellfun
    %     l1 = length(arrowX);
    %     for jl = 1:l1
    %         xs = arrowX{jl};
    %         ys = arrowY{jl};
    %         % example call
    %         % handle = arrowh(x,y,clr,ArSize,Where)
    %         arrowh([xs(2) xs(1)],[ys(2) ys(1)],'k');
    %     end
end

% Zooming on the Figure ?
if fullMode
...
if noFill
    ...
    function [splineX,splineY]=multi_Coords(splineX,splineY,colours,...
            tipPolyX,tipPolyY,tailPolyX,tailPolyY)
        ...
        if abs(asind((max(splineY(1,:))-min(splineY(1,:)))/(max(splineX(1,:))...
                -min(splineX(1,:)))))<40
        ...
        inPolygon=cellfun(@(a,b)inpolygon(a,b,tipPolyX,tipPolyY),...
            splineX,splineY,'uniformoutput',false);
        inPolygon2=cellfun(@(a,b)inpolygon(a,b,tailPolyX,tailPolyY),...
            splineX,splineY,'uniformoutput',false);
        inPolygon=cellfun(@not,cellfun(@or,inPolygon,inPolygon2,...
            'uniformoutput',false),'uniformoutput',false);
    ...
    end %funciton multi_Coords

    function multi_Plot(splineX,splineY,property,value,colours)
        cellfun(@(a,b,c)plot(a,b,property,value,'color',c),...
            splineX,splineY,colours);
    end %function multi_Plot
    ...
    function thisColour=getColour(thisColour,colourMode)
        ...
        if any(thisColour==58)
            ...
            thisColour=cellfun(@(a)getColour(a,colourMode),newColours,...
                'uniformoutput',false);
        ...
        if strmatch('#',thisColour)
            %hex code
            thisColour=[hex2dec(thisColour(2:3)) hex2dec(thisColour(4:5)) ...
                hex2dec(thisColour(6:7))];
            thisColour=thisColour./255;
            return;
        end
        if any(thisColour==32) %spaces - must be triplet.
            thisColour=str2num(thisColour); %#ok<ST2NM>
        ...
        try
            ...
            warning('mGraphViz:ColourUndefined',...
                ['Colour "' thisColour '" not defined. Please use hex values, '...
                'or edit the definitions manually to add it. Defaulting to white.']);
            thisColour=[1 1 1];
        end
    end %funciton getColour
    ...
    function [patchHandle]=...
            drawNode(style,x,y,width,height,edgeColour,bgColour,property,value)
        ...
        switch lower(style)
            ...
            case {'none' 'plaintext'}
                ...
                bgColour='none';
            case 'record'
                error('mGraphViz:ShapeNotSupported',...
                    ['The record node is not supported because the plain output'...
                    ' is not detailed enough to accurately draw such a node.']);
            otherwise
                patchX=[-0.5,0.5,0.5,-0.5];
                patchY=[0.5,0.5,-0.5,-0.5];
                edgeColour='none';
                bgColour='none';
                warning('mGraphViz:ShapeUndefined',['The shape ' style ...
                    ' is not defined. Please add me to the drawNode function!']);
                return;
        end
        if isInverted
            patchY=-patchY;
        ...
        if ~isempty(property)
            patchHandle=patch(patchX,patchY,[0 0 0],'FaceColor',bgColour,...
                'EdgeColor',edgeColour,property,value,'lineWidth',extraOutlines+1);
        else
            patchHandle=patch(patchX,patchY,[0 0 0],'FaceColor',bgColour,...
                'EdgeColor',edgeColour,'lineWidth',extraOutlines+1);
        end
