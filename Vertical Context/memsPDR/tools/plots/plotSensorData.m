...
% you can also define which sensor to read, output will be in engineering unit
    ...
    if sum(readOnlyMsgs == 10510) > 0
        if isfield(data, 'memsGTFloorCount')
            sensData.floorCount = data.memsGTFloorCount.data.floorCount;
        else
            sensData.floorCount = 0;
        end
    end
    
    if sum(readOnlyMsgs == 10504) > 0
        if isfield(data, 'memsGTContext')
            sensData.verticalContext = data.memsGTContext.data.context;
        else
            error('Data is missing Ground Truth.');
        end
    end
end
