function [state, results] =  runStepDetection(state, data, interval,logging)
results = emptyStepRes();
%Better to call I16 data here 
accdata = getSensorData(data.acc, interval, 3, 0);
for n=1:accdata.N
    ...
    if(accdata.valid(n)==1)
        ...
        if state.ifWalking == stepConsts.TRUE
            ...
            if state.samplesSinceLastStep~=stepConsts.STEP_EVENT_UPDATED
                if ifPeakDetected == stepConsts.POSITIVE_PEAK
                   ...
                   if state.sampleCount-lastStepEventDetected > 2*stepConsts.samplingRate
                       [~,index] = max(localPosPeaks(:,2));
                       lastStepEventDetected = state.sampleCount-stepConsts.averagingFilteredSearchWindow + startTag + localPosPeaks(index,1)-stepConsts.correctionDueToAveraging;
                   else
                       possibleStepEvents = state.sampleCount-stepConsts.averagingFilteredSearchWindow + startTag + localPosPeaks(:,1);
                       ...
                       %peak.
                       if length(localPosPeaks(:,1))>1
                           %check with previous peak
                           if index>1
                               previousPeakValue = localPosPeaks(index-1,2);
                               currentPeakValue = localPosPeaks(index,2);
                               durationDifference = localPosPeaks(index,1)-localPosPeaks(index-1,1);
                               if durationDifference<stepConsts.maxDurationDifferenceBetweenClosePeaks && previousPeakValue - currentPeakValue>stepConsts.minValueDifferenceBetweenClosePeaks
                                   index = index-1;
                               end
                           end
                           %check with next peak
                           if index+1<=length(localPosPeaks(:,1))
                               nextPeakValue = localPosPeaks(index+1,2);
                               currentPeakValue = localPosPeaks(index,2);
                               durationDifference = localPosPeaks(index+1,1)-localPosPeaks(index,1);
                               if durationDifference<stepConsts.maxDurationDifferenceBetweenClosePeaks && nextPeakValue - currentPeakValue>stepConsts.minValueDifferenceBetweenClosePeaks
                                   index = index+1;
                               end
                           end
                       end
                   ...
                   %Fine correction using acc norm
                   samplesFromEndOfWindow = (state.sampleCount-lastStepEventDetected);
                   normWindow = state.normAccAutoCovWindow(stepConsts.normAccAutoCovWinSize-samplesFromEndOfWindow-3:stepConsts.normAccAutoCovWinSize-samplesFromEndOfWindow+3);
                   [~,index] = max(normWindow);
                   index = index-5;
                   lastStepEventDetected = lastStepEventDetected + index;
                   
                   samplesFromStartOfWindow = n-(state.sampleCount-lastStepEventDetected);
                   results.nSteps = results.nSteps+1;
                   results.time = [results.time,accdata.t0 + samplesFromStartOfWindow*data.acc.info.sample_ms];
            ...
            %step frequency
            if state.walkingStartedDuration == stepConsts.upperLimitOnWalkingDuration && state.stepFrequency.confidence<stepConsts.leastConfidenceToKeepCurrentFilter
...
if(logging.dbgLogging.stepDetection>0)
    results.accdataT0 = accdata.t0;
    results.sampleCount = state.sampleCount;
...
if(accdata.N>0)
   state.stepResults.Time = accdata.t0 + (accdata.N-1)*data.acc.info.sample_ms;
end
end
...
%--Updated state
%-------------------
function state = ApplyAveragingFilter(state,inputData)
